...
**Known limitation 1 (documented, not a bug):** a production site whose *entire docroot* is a git repo (a root-level `.git`) is fully handled on the **first** pull — its root `.git` is neutralized to `.git.ferry-disabled` before the ferry repo is created. On a **re-pull**, the full transfer re-writes that root `.git` over the ferry repo's own `.git`; supporting this rare case (moving the ferry repo aside during transfer, or excluding a top-level `.git` upstream) is deferred. Nested plugin/theme repos — the case the base doc §5 actually calls out — are fully supported on every pull. The `.git/ferry-clone` sentinel makes this boundary explicit rather than silent.

**Known limitation 2 (apache only, surfaced by the Plan 2 whole-branch review):** on **apache**-parity clones, the v0 overlay's `finalizeClone` prepends the uploads-fallback block (`# BEGIN ferry-uploads-fallback …`) to the docroot `.htaccess`, which is **not** git-ignored. So on an apache clone (a) the committed `production` `.htaccess` carries the ferry block (the snapshot is not a perfectly pure mirror), and (b) if production ships no `.htaccess`, ferry creates one containing only the ferry block and the next pull's reconcile deletes it (tracked, absent from the manifest), breaking the fallback until re-created. **nginx clones are unaffected** — their fallback lives under the git-ignored `.ddev/`. The correct fix mirrors the nginx approach (serve the apache fallback from a git-ignored `.ddev/apache/` snippet instead of mutating the tracked `.htaccess`) and needs its own apache E2E; deferred until apache sites are explicitly supported. The shipped E2E validated the nginx path.
